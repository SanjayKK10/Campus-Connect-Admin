...
import { getCurrentAdminUser, getEventAttendees, getEventById, getEventRsvpCount, softDeleteEvent } from '../services/eventService';
...
export default function AdminEventDetail() {
  ...
  const loadAttendees = async () => {
    ...
    try {
      const rows = await getEventAttendees(eventId);
      setAttendees(rows);
    } catch (err) {
  ...
  const handleDeactivate = async () => {
    if (!event) return;
    const confirmed = window.confirm(`Deactivate “${event.title}”?`);
    if (!confirmed) return;
            ...
            <>
              ...
              <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {currentUserId && event.organizer_id === currentUserId ? (
                    <>
                      ...
                      <button type="button" onClick={handleDeactivate} className="rounded-xl border border-rose-200/70 bg-rose-50 px-4 py-2 text-sm font-medium text-rose-700">
                        Deactivate Event
                      </button>
